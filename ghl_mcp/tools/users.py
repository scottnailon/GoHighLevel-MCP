...
class UserGetInput(BaseToolInput):
...
class UserUpdateInput(BaseToolInput):
    user_id: str = Field(..., min_length=1)
...
def register(mcp) -> None:  # noqa: ANN001
    ...
    async def ghl_users_list(params: UsersListInput) -> str:
        ...
        result = await client.get("/users/", params={"locationId": location_id})
    ...
    async def ghl_users_get(params: UserGetInput) -> str:
        ...
        client = await get_client()
        result = await client.get(f"/users/{params.user_id}")
    ...
    async def ghl_users_create(params: UserCreateInput) -> str:
        ...
        result = await client.post("/users/", json=body)
    ...
    async def ghl_users_update(params: UserUpdateInput) -> str:
        ...
        result = await client.put(f"/users/{params.user_id}", json=body)
        return format_response(result, ResponseFormat.JSON)
